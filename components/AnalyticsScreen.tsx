...
import { AlertCircle, Award, Calendar, CheckCircle2, Home, Loader2, Percent, RefreshCw, TrendingUp, Trophy } from 'lucide-react';
...
const AnalyticsScreen: React.FC<AnalyticsScreenProps> = ({ onBack, playerData, userId }) => {
  ...
  if (isLoading && !player) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-emerald-50 gap-4">
      ...
      <p className="text-emerald-900 font-bold animate-pulse text-sm">جاري مزامنة بياناتك العلمية...</p>
    </div>
  );
  ...
  if (!player) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-emerald-50 gap-4 p-6 text-center">
      ...
      <div className="flex gap-3">
        ...
        <button onClick={fetchData} className="bg-emerald-600 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 text-sm"><RefreshCw size={18} /> تحديث</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-emerald-50/30 flex flex-col items-center p-3 sm:p-5 overflow-y-auto font-sans w-full pb-10">
      <div className="w-full max-w-2xl animate-fade-in-up">
        
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <button onClick={onBack} className="bg-white p-3 rounded-2xl shadow-sm text-slate-500 active:scale-95 border border-emerald-50">
            <Home size={22} />
          </button>
          ...
          <button onClick={fetchData} className="bg-white p-3 rounded-2xl shadow-sm text-slate-500 active:scale-95 border border-emerald-50">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-white p-4 rounded-3xl shadow-sm border border-emerald-50 flex flex-col items-center text-center">
            ...
            <div className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">الترتيب بالفصل</div>
          </div>

          <div className="bg-white p-4 rounded-3xl shadow-sm border border-emerald-50 flex flex-col items-center text-center">
            <div className="w-9 h-9 bg-emerald-100/50 rounded-full flex items-center justify-center text-emerald-600 mb-2">
              <CheckCircle2 size={18} />
            </div>
            <div className="text-2xl font-black text-slate-800">{totalCorrect}</div>
            <div className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">نقاط مجمعة</div>
          </div>
          ...
          <div className="bg-white p-4 rounded-3xl shadow-sm border border-emerald-50 flex flex-col items-center text-center">
            ...
            <div className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">سلسلة أيام</div>
          </div>
          ...
          <div className="bg-white p-4 rounded-3xl shadow-sm border border-emerald-50 flex flex-col items-center text-center">
            ...
            <div className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">دقة الإجابات</div>
          </div>
        </div>

        {/* Chart */}
        <div className="bg-white rounded-3xl shadow-sm p-5 mb-6 border border-emerald-50">
          <div className="flex items-center gap-2 mb-6">
            <div className="bg-emerald-50 p-2 rounded-xl text-emerald-600"><TrendingUp size={18} /></div>
            <h2 className="text-sm font-black text-slate-800">النشاط البحثي للأسبوع الماضي</h2>
          </div>
              ...
              return (
                <div key={idx} className="flex-1 flex flex-col items-center h-full justify-end">
                    <div className="w-full max-w-[24px] bg-slate-50 rounded-t-lg overflow-hidden flex flex-col-reverse relative h-full">
                        <div className="bg-emerald-100 w-full" style={{ height: `${barHeight}%` }}>
                            {total > 0 && <div className="bg-emerald-500 w-full" style={{ height: `${correctHeight}%` }}></div>}
                        </div>
                    </div>
        ...
        {/* Badges */}
        <div className="bg-white rounded-3xl shadow-sm p-5 border border-emerald-50">
          ...
          <div className="grid grid-cols-1 gap-3">
            {(player.badges || []).map((badge) => {
                ...
                return (
                <div key={badge.id} className={`p-4 rounded-2xl border-2 transition-all flex items-center gap-4 ${badge.unlocked ? `bg-emerald-50 border-emerald-100 shadow-sm` : 'bg-slate-50 border-transparent opacity-60'}`}>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[9px] font-bold mt-0.5 text-slate-500">
                      {badge.unlocked ? 'باحث معتمد!' : `باقي ${Math.max(0, badge.required - player.totalCorrect)} إجابة`}
                    </div>
                    {!badge.unlocked && (
